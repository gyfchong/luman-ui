  ...
  async run({ args }) {
    try {
      ...
      const cssPath = (await p.text({
        ...
        defaultValue: projectInfo.appDir ? "./src/app/globals.css" : "./src/styles/globals.css",
      })) as string
      ...
      if (componentCount > 0) {
        ...
        if (args.components) {
          ...
          if (componentsArg === "all") {
            ...
            componentsToInstall = componentsArg
              .split(",")
              .map((c) => c.trim())
              .filter(Boolean)
          }
          ...
          const selected = (await p.multiselect({
            ...
            options: [
              {
                value: "__all__",
                label: colors.cyan("All components") + colors.dim(` (${componentCount})`),
              },
        ...
        if (componentsToInstall.length > 0) {
          ...
          if (installed.length > 0) {
            installSpinner.stop(
              `${colors.green("✓")} Installed ${installed.length} component${installed.length === 1 ? "" : "s"}`
            )
          } else {
